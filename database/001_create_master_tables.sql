...
DROP TABLE IF EXISTS worksheet_forecast_average CASCADE;
...
-- =========================================================
-- Worksheet Forecast Average (summary / cache table)
-- =========================================================

-- Recalculated from worksheet_used so the Worksheet Forecast page does not
-- scan the full worksheet history on every request. worksheet_packet_no is a
-- plain packet number (no worksheet_master FK) so the cache survives packet
-- renumbering.

CREATE TABLE worksheet_forecast_average (
...
CREATE INDEX idx_worksheet_forecast_average_lookup
	ON worksheet_forecast_average
	(subject_id, level_master_id, worksheet_packet_no, source_scope);

CREATE INDEX idx_worksheet_forecast_average_calculated_at
	ON worksheet_forecast_average
	(calculated_at);
